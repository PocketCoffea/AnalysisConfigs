...
for i in range(len(eta_bins)-1):
    o=load(f"out_separate_eta_bin/eta{eta_bins[i]}to{eta_bins[i+1]}/output_all.coffea")
    # histos_dict = o["variables"]
    columns_dict = o["columns"]
    # median_dict[f"eta{eta_bins[i]}to{eta_bins[i+1]}"] = {}
    for j in range(len(pt_bins)-1):
        # var = f"MatchedJets_Response_eta{eta_bins[i]}to{eta_bins[i+1]}_pt{pt_bins[j]}to{pt_bins[j+1]}"
        var_column = f"MatchedJets_eta{eta_bins[i]}to{eta_bins[i+1]}_pt{pt_bins[j]}to{pt_bins[j+1]}_Response"
        for sample in columns_dict.keys():
            for dataset in columns_dict[sample].keys():
                ...
                # median_dict[f"eta{eta_bins[i]}to{eta_bins[i+1]}"][f"pt{pt_bins[j]}to{pt_bins[j+1]}"] = median
...
for i in range(len(eta_bins)-1):
    ...
    ax.legend()
    # plt.show()
    fig.savefig(f"median_plots/median_1d_eta{eta_bins[i]}to{eta_bins[i+1]}.png")
    fig.clf()
