...
def write_l2rel_txt(main_dir, correct_eta_bins, year, num_params):
    ...
    # create txt"""  """
    flav = "inclusive"
    file_name=f"{year}_PNETREG_MC_L2Relative_AK4PUPPI.txt"
        ...
        for i in range(len(correct_eta_bins) - 1):
            try:
                with open(
                    ...
                    params_string = ""
                    for param in fit_results_dict["inclusive_ResponsePNetRegNeutrino"][
                        "parameters"
                    ]:
                    ...
                    for j in range(
                        21
                        - len(
                            fit_results_dict["inclusive_ResponsePNetRegNeutrino"][
                                "parameters"
                            ]
                        )
                    ):
                        params_string += " 0"
                    jetpt_low = "    {}".format(
                        fit_results_dict["inclusive_ResponsePNetRegNeutrino"]["jet_pt"][
                            0
                        ]
                    )
                    jetpt_up = "    {}".format(
